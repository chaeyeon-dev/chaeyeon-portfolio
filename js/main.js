...
const projectData = {
  ...
  arduino: {
    ...
    info: [
      "온·습도 및 환경 센서 데이터 수집",
      "버튼 입력을 통한 창문 개폐 제어",
      "서보 모터를 활용한 창문 열림/닫힘 제어",
      "날씨 데이터 기반 옷차림·우산·마스크 정보 자동 표시",
      "App Inventor 기반 스마트폰 앱 연동",
    ],
  ...
  server: {
    ...
    info: [
      "키워드 입력을 통한 Naver OpenAPI 검색 기능",
      "API 응답(JSON/XML) 파싱 및 데이터 가공",
      "검색 결과를 <table> 형태로 화면 출력",
      "결과 데이터 DB 테이블 저장",
    ],
  ...
  bearlog: {
    title: "BearLog – 개발자 학습 관리 서비스",
    image: "images/bearlog.png",
    desc: "GitHub Issues/PR과 양방향 연동되는 개발자 전용 학습 관리 웹 서비스입니다. 일반 모드와 개발자 모드를 자유롭게 전환하며 학습 목표를 관리하고, 완료한 할 일은 캘린더에 곰발바닥 도장으로 기록됩니다. 디자이너·백엔드와 협업한 부트캠프 팀 프로젝트로, 기존 기획을 팀에서 재정의해 진행했습니다.",
    info: [
      "팀 단위 기획 재정의 및 요구사항 재설계 참여",
      "디자이너와 Figma 와이어프레임·디자인 시안 협업",
      "백엔드와 Swagger 기반 API 명세 공동 작성",
      "Button · Input · FormField · Tag 공통 컴포넌트 설계 및 Storybook 문서화",
      "랜딩 · 로그인 · 회원가입 페이지 UI 및 인터랙션 구현",
      "내 정보 관리 · 설정 · 알림 페이지 구현",
      "GitHub OAuth · Google OAuth 로그인 플로우 구현",
      "Next.js · TypeScript · Tailwind CSS · React Query · Storybook 활용",
      "Trunk-Based 브랜치 전략 및 커밋 컨벤션 기반 협업",
    ],
    link: "https://bearlog.vercel.app/",
  },
};
